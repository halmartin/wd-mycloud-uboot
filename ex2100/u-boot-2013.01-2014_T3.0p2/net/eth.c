...
int eth_init(bd_t *bis)
{
	...
	do {
		debug("Trying %s\n", eth_current->name);
#if 1       //ALPHA_CUSTOMIZE	for SGMII2 reason, we need to ignore egiga1
		/*
		 * We mapped egiga0->RGMII & egiga2->SGMII2.
		 * The init() will return sueecss when getting PortEn of egiga1 
		 * in GigaEthMacControl(0x32C00).
		 * But egiga1 is unavailable in actually.
		 * So we need to ignore egiga1 here.
		 */
		if(!strcmp(eth_current->name, "egiga1"))
			goto goto_eth_try;
#endif
